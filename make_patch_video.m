...
if nargin < 8
    ...
    down=zeros(1,4);
    for i = 1:4
        up(i) = max(max(A_com(:,:,i)))*max(C(ind(i),:));
        down(i)=max(max(A_com(:,:,i)))*min(C(ind(i),:));
        coffset(i)=min(C(ind(i),:));
    end
    ...
    for t = 1:skp:T
        ...
        for i = 1:4        
            subplot(4,6,14+i); imagesc(A_com(:,:,i)*(C(ind(i),t)-coffset(i)),[down(i),up(i)]); axis square; 
            ...
            subplot(4,6,20+i); imagesc(Y(int_x(i,:),int_y(i,:),t),[0,up(i)+mB]); axis square; axis off;
            if i == 2
