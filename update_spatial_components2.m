...
%     end
% end
maxGrowthIteration=P.maxGrowthIteration;
for tag=1:maxGrowthIteration
    
A_temp = reshape(full(A),d1,d2,nr);
expandCore = strel('square',4);
IND = zeros(d,nr);
for i = 1:nr
   BW3 = imdilate(A_temp(:,:,i),expandCore);
   IND(:,i) = BW3(:)>0;
end
Cf = [C;f];
